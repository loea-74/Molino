...
const OWNER = "loea-74";
const REPO = "Molino";
const FILE_PATH = "src/content/recipes.json";
const BRANCH = "main";
...
// GET — devuelve las recetas actuales desde GitHub
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ recipes: JSON.parse(content), sha: file.sha });
  } catch (e) {
    return NextResponse.json({ error: String(e) }, { status: 500 });
  }
}

// POST — guarda las recetas actualizadas en GitHub
  ...
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/${OWNER}/${REPO}/contents/${FILE_PATH}`,
      {
        ...
        body: JSON.stringify({
          message: "admin: actualizar recetas",
          content,
          sha,
          branch: BRANCH,
        }),
